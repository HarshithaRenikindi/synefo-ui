...
const SelectComponent = ({ lenses, placeholder, label, onLensChange, checkbox }) => {
  ...
  // Initialize with the first lens selected by default if selectedLenses is empty
  useEffect(() => {
    // Only update the selected lenses if none are selected
    if (lenses.length > 0 && selectedLenses.length === 0) {
      const defaultLens = lenses[0];
      setSelectedLenses([defaultLens]);
      onLensChange([defaultLens]); // Notify parent of the change

    }
  }, [lenses]); // Only depend on `lenses` to avoid re-triggering unnecessarily
  ...
  // Handle lens selection (single selection mode)
  const handleLensSelect = (lens) => {
    if (!checkbox) {
      setSelectedLenses([lens]);
      onLensChange([lens]); // Notify parent of the change
      setIsOpen(false); // Close dropdown after selection
    }
  };

  // Handle checkbox changes for multiple selection mode
  const handleCheckboxChange = (event, lens) => {
    const isChecked = event.target.checked;
    const updatedSelectedLenses = isChecked
      ? [...selectedLenses, lens]
    ...
    setSelectedLenses(updatedSelectedLenses);
    onLensChange(updatedSelectedLenses); // Notify parent of the change
  };
  ...
  return (
    <div className="mb-6" ref={dropdownRef}>
      ...
      <CustomSelectWrapper>
        <DropdownButton onClick={handleToggleDropdown}>
          {selectedLenses.length > 0
            ? selectedLenses.join(', ') // Show selected lenses or placeholder
            : placeholder}
          <Arrow isOpen={isOpen} />
        </DropdownButton>
