...
const InstanceHoursStopped = () => {
  ...
  const data = {
    labels: labels,
    datasets: [{
      label: 'Total Stopped Instance hours',
      data: [10, 15, 11, 7, 10, 15, 13, 7, 13, 11, 7, 14],  // Sample CPU utilization data
      fill: false,
      borderColor: 'rgb(255, 112, 139)',  // Color for CPU utilization line
  ...
  return (
    <div className='w-full bg-white py-2 px-4 rounded-md '> {/* Set the size of the container */}
      <h2>Instance Hours (Stopped)</h2>
      <p>Total Instance Hours (stopped) Over the Last month</p>
      <Line data={data} options={options} />
    </div>
  );
};

export default InstanceHoursStopped;
