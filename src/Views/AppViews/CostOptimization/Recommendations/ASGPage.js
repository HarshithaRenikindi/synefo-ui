...
function ASGRecommendationPage() {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      const asgResponse = await axios.get(
        ' http://localhost:3000/dev/recommendations/asg?accountId=657907747545&region=us-east-1'
      );
      setAutoScalingRecommendations(asgResponse.data);
  ...
  const handleFilterChange = (event) => {
    ...
    if (selectedFilter === 'Auto Scaling') {
    ...
    <div className="container">
      <div className='cost-optimization'>
        ...
        <div className="filter-container">
          <select value={filter} onChange={handleFilterChange}>
            <option value="All Resources">Auto Scaling</option>
            <option value="EC2">EC2</option>
            <option value="EBS">EBS</option>
            <option value="RDS DB">RDS DB</option>
            <option value="Lambda">Lambda</option>
            <option value="Auto Scaling">All Resources</option>
          </select>
        </div>
      </div>
      ...
      <div className='asg-container'>
        <div>
          ...
          {autoScalingRecommendations.length > 0 ? (
            <table className="recommendations-table">
              ...
              <tbody>
                {autoScalingRecommendations.map((rec, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        type="checkbox"
...
export default ASGRecommendationPage;
